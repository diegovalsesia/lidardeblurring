...
class Embeddings_output(nn.Module):
    def __init__(self):
        ...
        self.depth_block2_2 = EmbedDepthTransform(192)
        self.adapter2_2 = DepthPromptMoudle(192)
        ...
        self.depth_block1_3 = DepthTransformDecoder(192,64)
        self.de_layer1_3 = nn.Sequential(
            nn.Conv2d(128, 64, kernel_size=1, padding=0),
            self.activation,
            nn.Conv2d(64, 64, kernel_size=3, padding=1))
        self.adapter1_3 = DepthPromptMoudle(64)
    ...
    def forward(self, x, dep, residual_1, residual_2):
        dep = self.up_net(dep)
        en_dep = self.depth_block2_2(dep)

        hx = self.de_layer3_1(x)

        hx = self.de_layer2_2(torch.cat((hx, residual_2), dim = 1))
        _,_,h,w = hx.size()
        en_dep = F.interpolate(en_dep,size=( h, w))
        hx = self.adapter2_2(en_dep, hx)
        ...
        hx_skip = self.de_layer1_3(torch.cat((hx,residual_1), dim=1))
        en_dep = self.depth_block1_3(en_dep)
        hx_skip = self.adapter1_3(en_dep, hx_skip)
        hx = self.activation(hx_skip + hx)
...
class DepthStripformer(nn.Module):
    def __init__(self):
        super(DepthStripformer, self).__init__()
    ...
    def forward(self, x, depth):
        ...
        hx = self.decoder(hx, depth, residual_1, residual_2)
